...
func TestApplyChanges(t *testing.T) {
	...
	filePath := filepath.Join(relativePathTestDir, "pacman_game.py")
	content := "import pygame\nimport random\n\n# Initialize Pygame\npygame.init()\n\n# Screen dimensions\nSCREEN_WIDTH = 800\nSCREEN_HEIGHT = 600\n\n# Colors\nBLACK = (0, 0, 0)\nWHITE = (255, 255, 255)\nYELLOW = (255, 255, 0)\nRED = (255, 0, 0)\n+ GREEN = (0, 255, 0)\n\n# Pacman settings\nPACMAN_SIZE = 20\nPACMAN_SPEED = 5\n\n# Ghost settings\nGHOST_SIZE = 20\nGHOST_SPEED = 3\n\n+ # Dot settings\n+ DOT_SIZE = 10\n+ DOT_COUNT = 20\n\n# Create the screen\nscreen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))\npygame.display.set_caption(\"Pacman\")\n\n# Pacman class\nclass Pacman:\n    def __init__(self):\n        self.x = SCREEN_WIDTH // 2\n        self.y = SCREEN_HEIGHT // 2\n        self.speed = PACMAN_SPEED\n        self.direction = None\n+       self.score = 0\n\n    def move(self):\n        if self.direction == \"UP\":\n            self.y -= self.speed\n        elif self.direction == \"DOWN\":\n            self.y += self.speed\n        elif self.direction == \"LEFT\":\n            self.x -= self.speed\n        elif self.direction == \"RIGHT\":\n            self.x += self.speed\n\n        # Keep Pacman within screen bounds\n        self.x = max(0, min(self.x, SCREEN_WIDTH - PACMAN_SIZE))\n        self.y = max(0, min(self.y, SCREEN_HEIGHT - PACMAN_SIZE))\n\n    def draw(self):\n        pygame.draw.circle(screen, YELLOW, (self.x, self.y), PACMAN_SIZE // 2)\n\n+    def collect_dot(self, dot):\n+        if abs(self.x - dot.x) < PACMAN_SIZE and abs(self.y - dot.y) < PACMAN_SIZE:\n+            return True\n+        return False\n\n# Ghost class\nclass Ghost:\n    def __init__(self):\n        self.x = random.randint(0, SCREEN_WIDTH - GHOST_SIZE)\n        self.y = random.randint(0, SCREEN_HEIGHT - GHOST_SIZE)\n        self.speed = GHOST_SPEED\n        self.direction = random.choice([\"UP\", \"DOWN\", \"LEFT\", \"RIGHT\"])\n\n    def move(self):\n        if self.direction == \"UP\":\n            self.y -= self.speed\n        elif self.direction == \"DOWN\":\n            self.y += self.speed\n        elif self.direction == \"LEFT\":\n            self.x -= self.speed\n        elif self.direction == \"RIGHT\":\n            self.x += self.speed\n\n        # Change direction if ghost hits the screen bounds\n        if self.x <= 0 or self.x >= SCREEN_WIDTH - GHOST_SIZE:\n            self.direction = random.choice([\"UP\", \"DOWN\", \"LEFT\", \"RIGHT\"])\n        if self.y <= 0 or self.y >= SCREEN_HEIGHT - GHOST_SIZE:\n            self.direction = random.choice([\"UP\", \"DOWN\", \"LEFT\", \"RIGHT\"])\n\n    def draw(self):\n        pygame.draw.rect(screen, RED, (self.x, self.y, GHOST_SIZE, GHOST_SIZE))\n\n+ # Dot class\n+ class Dot:\n+     def __init__(self):\n+         self.x = random.randint(0, SCREEN_WIDTH - DOT_SIZE)\n+         self.y = random.randint(0, SCREEN_HEIGHT - DOT_SIZE)\n+\n+     def draw(self):\n+         pygame.draw.circle(screen, GREEN, (self.x, self.y), DOT_SIZE // 2)\n\n# Main game loop\ndef main():\n    clock = pygame.time.Clock()\n    pacman = Pacman()\n    ghosts = [Ghost() for _ in range(5)]\n+    dots = [Dot() for _ in range(DOT_COUNT)]\n    running = True\n\n    while running:\n        for event in pygame.event.get():\n            if event.type == pygame.QUIT:\n                running = False\n            elif event.type == pygame.KEYDOWN:\n                if event.key == pygame.K_UP:\n                    pacman.direction = \"UP\"\n                elif event.key == pygame.K_DOWN:\n                    pacman.direction = \"DOWN\"\n                elif event.key == pygame.K_LEFT:\n                    pacman.direction = \"LEFT\"\n                elif event.key == pygame.K_RIGHT:\n                    pacman.direction = \"RIGHT\"\n\n        # Move Pacman and ghosts\n        pacman.move()\n        for ghost in ghosts:\n            ghost.move()\n\n+        # Check for dot collection\n+        for dot in dots[:]:\n+            if pacman.collect_dot(dot):\n+                pacman.score += 10\n+                dots.remove(dot)\n\n        # Check for collisions\n        for ghost in ghosts:\n            if abs(pacman.x - ghost.x) < PACMAN_SIZE and abs(pacman.y - ghost.y) < PACMAN_SIZE:\n                running = False\n\n        # Draw everything\n        screen.fill(BLACK)\n        pacman.draw()\n        for ghost in ghosts:\n            ghost.draw()\n+        for dot in dots:\n+            dot.draw()\n\n+        # Display score\n+        font = pygame.font.Font(None, 36)\n+        score_text = font.render(f\"Score: {pacman.score}\", True, WHITE)\n+        screen.blit(score_text, (10, 10))\n\n        pygame.display.flip()\n\n        # Cap the frame rate\n        clock.tick(30)\n\n    pygame.quit()\n\nif __name__ == \"__main__\":\n    main()"
